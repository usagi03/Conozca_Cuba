...
export class Km_hours_costsService {
  ...
  async getKm_hours_costs() {
    const khs = await this.km_hours_costRepository.find();
  
    const enhancedKhs = await Promise.all(khs.map(async (k) => {
      const model = await this.modelService.getModel(k.id_transp_model);
      return {...k, model };
    }));
  ...
  async getKm_hours_cost(id: number) {
    const km_hours_cost = await this.km_hours_costRepository.findOne({ where: { id_transp_model: id } });
    ...
    const model = await this.modelService.getModel(km_hours_cost.id_transp_model);
  
    return {...km_hours_cost, model };
  }  
  
  async createKm_hours_cost(newKm_hours_cost: CreateKm_hours_costDto): Promise<Km_hours_cost> {
    const model = this.modelService.createModel(newKm_hours_cost.type_transp_model, newKm_hours_cost.description_tm);
    console.log(model);
  ...
  async updateKm_hours_cost(id_transp_model: number, newKm_hours_cost: UpdateKm_hours_costDto) {
    ...
    await this.modelService.updateModel(id_transp_model, newKm_hours_cost.description_tm);
    ...
    km.extra_km_cost = newKm_hours_cost.extra_km_cost;
    km.extras_hours_cost = newKm_hours_cost.extras_hours_cost;
    const affectedRows = await this.km_hours_costRepository.update(id_transp_model, km);
