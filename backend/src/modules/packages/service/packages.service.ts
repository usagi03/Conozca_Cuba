...
export class PackagesService {
    ...
    async getPackage(id: number): Promise<Package> {
        console.log(id);
        const pack = await this.packageRepository.findOne({ relations: ['contract'], where: { id_package: id } } as FindOneOptions<Package>);
        console.log(pack);
    ...
    async createPackage(newPackage: CreatePackageDto): Promise<Package> {
        ...
        pack.contract = { id_contract: newPackage.contract.id_contract } as Contract; 
